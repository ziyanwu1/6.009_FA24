...
function handle_visible_button(){
  if (!game_over) {
    if (!visible_state) {
      document.getElementById('visible_button').innerHTML = "visible ON";
      visible_state = true;
    }
    else {
      document.getElementById('visible_button').innerHTML = "visible OFF";
      visible_state = false;
    }
    var args = {
      "game": board_state,
...
function render_result_dig(result) {
  ...
  if (result[0] == "victory") {
    ...
    document.getElementById('gameStateText').innerHTML = "YOU LOSE - YOU DUG A MINE!";
    game_over = true;
    var args = {
      "all_visible": true,
    "num_rows": board_params[board_size],
    ...
    document.getElementById('gameStateText').innerHTML = "GOOD MOVE - YOU DUG " + result[1] + " SQUARES!";
  }
  else {
...
function dig_square(row, col){
  ...
  invoke_rpc("/ui_dig_2d", args, 0, render_result_dig);
}
...
function square_style_mine(x, y){
    context.beginPath();
